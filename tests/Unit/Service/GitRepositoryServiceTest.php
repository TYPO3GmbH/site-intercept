...
class GitRepositoryServiceTest extends TestCase
{
    ...
    public function testComposerJsonUrlIsResolvedByPayload(string $expectedUrl, \stdClass $payload, string $repoService): void
    {
        ...
        self::assertSame($expectedUrl, $composerJsonUrl, 'Resolved composer json url did not match.');
    }
    ...
    public static function payloadDataProvider(): array
    {
        return [
            'Bitbucket (cloud) push' => [
                ...
                'payload' => self::getPayloadFromFixture('Payload_Bitbucket_Cloud_Event_Push.json'),
            ...
            'Bitbucket (server) push' => [
                ...
                'payload' => self::getPayloadFromFixture('Payload_Bitbucket_Server_Event_Push.json'),
                'repoService' => GitRepositoryService::SERVICE_BITBUCKET_SERVER,
            ],
            'Bitbucket (server) push with tag' => [
                'expectedUrl' => 'https://bitbucket.typo3.com/projects/EXT/repos/querybuilder/raw/composer.json?at='
                    . urlencode('refs/tags/v1.1.1'),
                'payload' => self::getPayloadFromFixture('Payload_Bitbucket_Server_Event_Push_Tag.json'),
                'repoService' => GitRepositoryService::SERVICE_BITBUCKET_SERVER,
            ],
            'GitHub (cloud) push branch' => [
                'expectedUrl' => 'https://raw.githubusercontent.com/Codertocat/Hello-World/main/composer.json',
                'payload' => self::getPayloadFromFixture('Payload_GitHub_Event_Push_Branch.json'),
            ...
            'GitHub (cloud) push tag' => [
                ...
                'payload' => self::getPayloadFromFixture('Payload_GitHub_Event_Push_Tag.json'),
                'repoService' => GitRepositoryService::SERVICE_GITHUB,
            ],
            'GitLab (cloud) push branch' => [
                'expectedUrl' => 'http://example.com/mike/diaspora/raw/main/composer.json',
                'payload' => self::getPayloadFromFixture('Payload_Gitlab_Event_Push_Branch.json'),
            ...
            'GitLab (cloud) push tag' => [
                ...
                'payload' => self::getPayloadFromFixture('Payload_Gitlab_Event_Push_Tag.json'),
                'repoService' => GitRepositoryService::SERVICE_GITLAB,
            ],
        ];
    }

    public static function filterAllowedBranchesDataProvider(): array
    {
        return [
