...
{
    final public const SERVICE_BITBUCKET_CLOUD = 'bitbucket-cloud';
    ...
    public function filterAllowedBranches(array $branchesAndTags): array
    {
        ...
        foreach ($branchesAndTags as $item) {
            ...
            if (preg_match('/^v?(\d+.\d+.\d+)$/', (string) $item)) {
                $versions[] = $item;
            }
        }
        $uniqueVersion = [];
        ...
        foreach ($versions as $version) {
            $versionParts = explode('.', ltrim((string) $version, 'v'));
            $shortVersion = $versionParts[0] . '.' . $versionParts[1];
            if (!isset($uniqueVersion[$shortVersion]) || version_compare(ltrim((string) $version, 'v'), ltrim((string) $uniqueVersion[$shortVersion], 'v')) > 0) {
